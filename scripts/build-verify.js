...
async function verifyMainBuild() {
  ...
  log.success(`主构建成功，耗时: ${(buildResult.time / 1000).toFixed(2)}s`);
...
async function verifyPlatformBuilds() {
  ...
  for (const platform of platforms) {
    ...
    try {
      ...
      log.warning(`${platform.name} 构建跳过: ${error.message}`);
    }
  }
...
function generateBuildReport(mainBuild, platformBuilds, benchmarks) {
  ...
  console.log(`  平台构建: ${report.summary.successfulPlatforms}/${report.summary.totalPlatforms} 成功`);
...
async function main() {
  ...
  try {
    ...
    log.error(`构建验证过程中发生错误: ${error.message}`);
    process.exit(1);
  }
}
