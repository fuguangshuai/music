...
function generateReport() {
  ...
  testResults.overall.totalDuration = 
    testResults.unit.duration + 
    testResults.integration.duration + 
    testResults.e2e.duration;
  ...
  console.log('');
  console.log('详细结果:');
  console.log(`• 单元测试: ${testResults.unit.passed ? '✅ 通过' : '❌ 失败'} (覆盖率: ${testResults.unit.coverage}%)`);
  console.log(`• 集成测试: ${testResults.integration.passed ? '✅ 通过' : '❌ 失败'} (覆盖率: ${testResults.integration.coverage}%)`);
  console.log(`• 端到端测试: ${testResults.e2e.passed ? '✅ 通过' : '❌ 失败'} (覆盖率: ${testResults.e2e.coverage}%)`);
...
async function runTests() {
  try {
    ...
    // 根据结果设置退出码
    const allPassed = testResults.unit.passed && 
                     testResults.integration.passed && 
                     testResults.e2e.passed && 
                     testResults.overall.passed;
