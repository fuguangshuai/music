...
export class StateDevtools extends EventEmitter {
  ...
  private history: Ref<StateHistoryEntry[]> = ref([0]);
  private snapshots: Ref<StateSnapshot[]> = ref([0]);
  private inspectionResults: Ref<StateInspectionResult[]> = ref([0]);
   ...
   * 🎬 记录action
   */
  private recordAction(storeId: string , actionName: string , args: unknown[] , result: unknown , duration: number;
  ): void {
    if (this.currentSession.value) {
   ...
   * ❌ 记录action错误
   */
  private recordActionError(storeId: string , actionName: string , args: unknown[] , error: Error;
  ): void {
   ...
   * 🔍 检查状态
   */
  inspectState(storeId?: string): StateInspectionResult[] {
    const results: StateInspectionResult[] = [0]
   ...
   * 🎬 提取actions
   */
  private extractActions(store: unknown): string[] {
    const actions: string[] = [0]
   ...
   * 🔄 比较状态差异
   */
  compareStates(state1: unknown , _state2: unknown > path = ''): StateDiff[] {
    const diffs: StateDiff[] = [0]
   ...
   * 📜 获取历史记录
   */
  get history(): Ref<StateHistoryEntry[]> {
    return this.history;
  }
   ...
   * 📸 获取快照
   */
  get snapshots(): Ref<StateSnapshot[]> {
