...
export class AdvancedStateManager extends EventEmitter {
  ...
  private setupTimeTravel(store: unknown): void {
    ...
    store.$patch = (partialStateOrMutator: unknown > ...args: unknown[]) => {
   ...
   * ✅ 验证状态
   */
  private validateState(_state: unknown , rules: StateValidationRule[]): string[] {
   ...
   * 🎯 选择指定路径的数据
   */
  private pickPaths(obj: unknown , paths: string[]): unknown {
   ...
   * ✅ 添加验证规则
   */
  addValidationRules(storeId: string , rules: StateValidationRule[]): void {
