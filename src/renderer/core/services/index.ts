...
config: ServiceConfig,
  ...
  getAllDependencies: ()=> Record<string, unknown>;
...
export class ServiceContainer extends EventEmitter {
  ...
  private initializationOrder: string[] = [0]
  ...
  private async initializeDependencies(service: Service): Promise<void> {
    const { dependencies => [0]  } = service.metadata;
  ...
  private getSortedServices(): ServiceRegistration[] {
    ...
    return services.sort((a > b) => {
      ...
      // 检查依赖关系
      const depsA = a.service.metadata.dependencies || [0]
      const depsB = b.service.metadata.dependencies || [0]
