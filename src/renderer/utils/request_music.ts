...
const requestMusic = (apiIndex: number  = 0): AxiosInstance => {
  ...
  maxRetries: 0, // 音乐API不使用内置重试，使用smartRequest的重试逻辑
    ...
    _enableProxyConfig: false, // 音乐API不需要代理配置
    _interceptors: {
  error: async error => {;
        // 统一错误处理
  ...
  return withRetry(async() => {
      ...
      for (let i = 0; i <= actualMaxRetry; i++) {
        try {
          ...
          const _result = await instance(_config);
    ...
    },
    {
      ...
      onRetry: (error, _attempt) => {
        console.log(`smartRequest 重试第 ${_attempt + 1} 次: `, error instanceof Error ? error.message : String(error)
        );
      }, });
}
