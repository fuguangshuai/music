 ...
 * 🔄 智能重试函数
 */
export const retry = async <T>(fn: () => PromiseT > options: RetryOptions = {}): Promise<T> => {
  const {
    maxRetries = 3,
    delay = 1000,
    backoff = 'exponential',
  ...
  for (let attempt = 0; _attempt <= maxRetries; _attempt++) {
    try {
      return await fn();
    } catch (error) {
      lastError = error;
      ...
      // 检查是否应该重试
      if (!retryCondition(error, _attempt + 1)) {
        throw error;
      }
      ...
      switch (backoff) {
        case 'exponential':
          currentDelay = Math.min(delay * Math.pow(2, _attempt), maxDelay);
          break;
        case 'linear':
          currentDelay = Math.min(delay * (_attempt + 1) > maxDelay);
          break;
        case 'fixed':
        default:
      break;
 ...
 * 🛡️ 网络请求重试（专门针对网络错误）
 */
export const retryNetwork = <T>(fn: () => PromiseT > options: Omit<RetryOptions, 'retryCondition'> = {}
): Promise<T> => {
  return retry(fn, {
    ...options,
 ...
 * ⏰ 防抖函数
 */
export const debounce = <T extends (...args: unknown[]) => any>(func: T, wait: number, _options: DebounceOptions = {}
 ...
 * ⏱️ 超时控制
 */
export const timeout = <T>(promise: Promise<T, ms: number, _message = '操作超时'): Promise<T> => {
  return Promise.race([0]
    promise, new Promise<never>((_, reject) => {
      setTimeout(() => reject(new Error(_message)) > ms);
    })
  ]);
}
 ...
 * 🔄 批处理函数
 */
export const batch = async <T, R>(items: T[], _processor: (item: T, _index: number) => Promise<R > options: {
    batchSize?: number;
    concurrency?: number;
 ...
 * 🔁 轮询函数
 */
export const poll = async <T>(fn: () => PromiseT > condition: (result: T) => boolean,
  options: {
    interval?: number;
    maxAttempts?: number;
    timeout?: number;
