  ...
  return function (_target: unknown, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function (...args: unknown[]) {
      try {
        ...
        // 尝试从缓存获取
        const cached = await smartCacheService.getCachedData(options.type, cacheKey);
        if (cached !== undefined) {
          return cached;
        }
...
export class CacheUtils {
  ...
  static async cacheImage(url: string, imageData: string | Blob, ttl?: number): Promise<boolean> {
    ...
    return await smartCacheService.cacheData(CacheType.IMAGE, key, imageData, ttl);
  }

  static async getCachedImage(url: string): Promise<string | Blob | undefined> {
    const key = this.generateImageKey(url);
    return await smartCacheService.getCachedData(CacheType.IMAGE, key);
  }
  ...
  static async getCachedAudioMetadata(songId: string): Promise<Record<string, unknown> | undefined> {
  ...
  static async cacheApiResponse(
    ...
    return await smartCacheService.cacheData(CacheType.API_RESPONSE, key, response, ttl);
  }
  ...
  static async getCachedApiResponse(
    ...
    const key = this.generateApiKey(endpoint, params);
    return await smartCacheService.getCachedData(CacheType.API_RESPONSE, key);
  }
  ...
  static async analyzeCachePerformance(): Promise<{
    ...
    try {
      const totalStats = await smartCacheService.getCacheStats() as Map<string, CacheStats> | null;
...
export const CacheDecorators = {
   ...
   * API响应缓存装饰器
   */
  apiResponse: (ttl: number = 5 * 60 * 1000) => cached({
   ...
   * 图片缓存装饰器
   */
  image: (ttl: number = 60 * 60 * 1000) => cached({
   ...
   * 用户数据缓存装饰器
   */
  userData: (ttl: number = 30 * 60 * 1000) => cached({
