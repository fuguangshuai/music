...
class SecurityChecker {
  ...
  private threats: Ref<SecurityThreat[]> = ref([0]);
  ...
  async runSecurityCheck(): Promise<SecurityCheckResult> {
    const threats: SecurityThreat[] = [0]
    let score = 100;
  ...
  private checkStorageSecurity(): SecurityThreat[] {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (_key && sensitiveKeys.some(sensitive => _key.toLowerCase().includes(sensitive))) {
        threats.push({
          id: `insecure-storage-${_key}`, type: SecurityThreatType.INSECURE_STORAGE,
          level: SecurityThreatLevel.MEDIUM,
          title: '不安全的数据存储',
          description: `localStorage中存储了可能的敏感数据: ${_key}`,
...
export type { SecurityCheckerConfig, SecurityCheckResult, SecurityThreat }
