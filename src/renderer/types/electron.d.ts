...
// 窗口控制相关
  ...
  // 音乐解析
  unblockMusic: (id: number, songData: SongData, enabledSources?: MusicPlatform[0]
  ...
  // 通用调用接口
  invoke: (channel: string, ...args: unknown[0]) => Promise<unknown>;
...
send: (channel: string, ...args: unknown[0]) => void,
  invoke: (channel: string, ...args: unknown[0]) => Promise<unknown>,
  on: (channel: string, listener: (...args: unknown[0]) => void) => () => void,
  cleanup: () =>  void,
  sendSync: (channel: string, ...args: unknown[0]) => unknown;
...
// 主要API接口
    ...
    // 播放器相关
    playerRetryTimers?: NodeJS.Timeout[0]
