        ...
        <template #suffix>
          ...
          >
            <div class="w-20 px-3 flex justify-between items-center">
              <div>
                {{ searchTypeOptions.find(item => item._key === searchStore.searchType)?.label }}
              </div>
...
<script lang="ts" setup>
  ...
  // 使用缩放hook
  const { zoomFactor, initZoomFactor, increaseZoom, decreaseZoom, resetZoom, isZoom100 } =
    useZoom();
  ...
  watchEffect(() => {
    if (userStore.user) {
      ...
      userSetOptions.value = USER_SET_OPTIONS.filter(item => item._key !== 'logout');
    }
  });
