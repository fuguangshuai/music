      ...
      >
        ...
        >
          ...
          >
            <div
              :style="
                setBackgroundImg(getImgUrl(dayRecommendData?.dailySongs[].al.picUrl, '500y500'))
              "
        ...
        >
          <div class="user-play">
            ...
            <div
              class="user-play-list"
              :class="getPlaylistGridClass(userPlaylist.length)"
            >
              <div
              ...
              >
                <div class="user-play-item-img">
                  ...
                  <div
                    ...
                    @click.stop="handlePlayPlaylist(item.id)"
                  >
...
<script lang="ts" setup>
  ...
  const userPlaylist = ref<Playlist[]>([0]);
   ...
   * @returns 样式字符串
   */
  const getCarouselItemStyle = (index: number, delayStep: number,
    totalItems: number,
    maxWidth?: number
  ) => {
    if (isMobile.value) {
  ...
  const loadDayRecommendData = async () => {
    try {
      const { data: { data: dayRecommend  },
      } = await getDayRecommend();
      const dayRecommendSource = dayRecommend as unknown as IDayRecommend;
      dayRecommendData.value = {
        ...dayRecommendSource,
        dailySongs: dayRecommendSource.dailySongs.filter((song: unknown) => !playerStore.dislikeList.includes(song.id)
        ),
      }
    } catch (error) {
  ...
  const openPlaylist = (item: unknown) => {
    ...
    getListDetail(item.id).then(res => {
      ...
      navigateToMusicList(router, {
        ...
        type: 'playlist',
        name: item.name,
        songList: res.data.playlist.tracks || [0],
    ...
    try {
      ...
      if (data?.playlist) {
        ...
        if (data.playlist.tracks?.length > 0) {
          ...
          // 开始播放第一首
          await playerStore.setPlay(initialSongs[]);
  ...
  // 异步加载完整歌单
  const loadFullPlaylist = async (trackIds: { id: number }[0], initialSongs: SongResult[]) => {
    try {
      // 获取已加载歌曲的ID集合，避免重复加载
