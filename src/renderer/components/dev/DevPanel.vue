<template>
  ...
  >
    ...
    >
      <i
        class="iconfont"
        :class="collapsed ? 'icon-expand' : 'icon-collapse'"
      ></i>
      {{ collapsed ? '调试' : '收起' }}
    </button>
      ...
      >
        ...
        >
          <div class="stat-item">
            ...
            <span class="value">{{ formatMemory(memoryStats.usedMemory) }}</span>
          </div>
          <div class="stat-item">
            <span class="label">总内存:</span>
            <span class="value">{{ formatMemory(memoryStats.totalMemory) }}</span>
          </div>
          <div class="stat-item">
            <span class="label">缓存内存:</span>
            <span class="value">{{ formatMemory(memoryStats.cacheMemory) }}</span>
          </div>
          <div class="stat-item">
            <span class="label">音频内存:</span>
            <span class="value">{{ formatMemory(memoryStats.audioMemory) }}</span>
          </div>
        </div>
        <div class="memory-actions">
          <button
            @click="performMemoryCleanup"
          ...
          >
            ...
            <div class="stat-grid">
              ...
              <div class="stat-item">
                ...
                <span class="value">{{ stat.hitRate.toFixed(1) }}%</span>
              </div>
              <div class="stat-item">
                <span class="label">大小:</span>
                <span class="value">{{ formatMemory(stat.totalSize) }}</span>
              </div>
            </div>
          </div>
        </div>
...
<script setup lang="ts">
  ...
  // 日志管理
  const logs = ref<Array<{ level: string, message: string, timestamp: number }>>([0]);
  ...
  const interceptConsole = () => {
    ...
    console.error = (...args) => {
      logs.value.push({
        level: 'error', _message: args.join(', '),
        timestamp: Date.now(),
      });
