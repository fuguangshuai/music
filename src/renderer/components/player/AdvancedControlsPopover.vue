<template>
    ...
    >
      <template #trigger>
        <div class="advanced-controls-btn">
          ...
          <!-- 激活状态的小标记 -->
          <div
            v-if="hasActiveSettings"
            class="active-indicator"
          >
            <span
              v-if="hasActiveSleepTimer"
              class="timer-badge"
            >
  ...
  >
    <div class="timer-modal-content">
      <div
        class="modal-close"
        @click="playerStore.showSleepTimer = false"
      >
        <i class="ri-close-line"></i>
      </div>
      <sleep-timer />
...
<script lang="ts" setup>
  ...
  watch(() => playerStore.showSleepTimer,
    newValue => {
      if (newValue) {
        // 如果睡眠定时器弹窗打开，关闭其他弹窗
  ...
  const dropdownOptions = computed<DropdownOption[]>(() => [
    ...
    },
    {
      ...
      // 如果有激活的定时器，添加标记
      suffix: () => null
        hasActiveSleepTimer.value ? h('span', { class: 'active-option-mark' }) : null,
    },
    {
