        ...
        >
          ...
          >
            ...
            >
              ...
              <div
                v-if="config.showTranslation && item.trText"
                class="translation"
              >
                {{ item?.trText }}
              </div>
...
<script setup lang="ts">
  ...
  const scrollToCurrentLyric = (immediate = false, customScrollerRef?: HTMLElement | null) => {
    try {
      ...
      scrollerRef.scrollTo({
        top: scrollTop, behavior: immediate ? 'auto' : 'smooth',
      });
    } catch (err) {
  ...
  const handleMouseDown = (e: MouseEvent) => {
    ...
    // 立即更新位置
    const progressBar = (e.currentTarget as HTMLElement).closest('.apple-style-progress'
  ,  ) as HTMLElement;
    if (progressBar) {
      const rect = progressBar.getBoundingClientRect();
      const offsetX = e.clientX - rect.left;
  ...
  const togglePlayMode = () => {
    ...
    showBottomToast( [t('player.playMode.sequence'), t('player.playMode.loop'), t('player.playMode.random')][
        playMode.value
      ]
    );
  }
  ...
  #mobile-drawer-target {
    ...
    .record-style-common {
      ...
      &::before {
        ...
        background: radial-gradient(; circle at center, transparent 38%,
          rgba(0, 0, 0, 0.15) 38%,
          rgba(0, 0, 0, 0.15) 39%,
      ...
      .img-wrapper {
        ...
        &::after {
          ...
          background: linear-gradient(;
            135deg, rgba(255, 255, 255, 0.05) 0%,
      ...
      .landscape-layout {
        ...
        .landscape-lyrics-section {
          ...
          .landscape-lyrics-scroller {
            ...
            mask-image: linear-gradient(to bottom, transparent 5%,
              black 15%,
              black 85%,
              transparent 95%
            );
            -webkit-mask-image: linear-gradient(to bottom, transparent 5%,
              black 15%,
              black 85%,
              transparent 95%
            );
  ...
  .cover-container {
    ...
    &.full-style {
      ...
      &::after {
        ...
        right: 0,
        height: 40%,
        background: linear-gradient(; transparent, var(--bg-color, rgba(25, 25, 25, 1)) 70%,
