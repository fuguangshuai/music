...
<script setup lang="ts">
  ...
  // 监听设置组件的配置变化
  watch(() => lyricSettingsRef.value?.config,
    newConfig => {
      if (newConfig) {
  ...
  // 监听字体变化并更新 CSS 变量
  watch(() => [setData.value.fontFamily, setData.value.fontScope],
    ([newFont, fontScope]) => {
      const defaultFonts =
  ...
  // 添加文字间距监听
  watch(() => config.value.letterSpacing,
    newSpacing => {
