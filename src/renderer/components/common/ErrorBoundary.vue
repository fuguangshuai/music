<template>
  <div class="error-boundary">
    ...
    <div v-else class="error-boundary__content">
      <!-- 用户友好的错误界面 -->
      <div class="error-boundary__user-view" v-if="!showDetails">
        ...
        <div class="error-actions">
          ...
          <n-button quaternary @click="handleReload">
            <i class="iconfont icon-reload"></i>
            刷新页面
          </n-button>
          <n-button quaternary @click="showDetails = true" v-if="isDev">
      ...
      <!-- 开发者详细错误信息 -->
      <div class="error-boundary__dev-view" v-if="showDetails && isDev">
        <div class="error-details">
          ...
          <div class="error-info">
            ...
            <div class="error-field">
              <label>发生时间:</label>
              <span>{{ formatTime(errorInfo?.timestamp || 0) }}</span>
            </div>
            <div class="error-field">
              <label>组件路径:</label>
        ...
        <div class="dev-actions">
          ...
          <n-button @click="reportError" type="warning">
...
const userFriendlyMessage = computed(() => {
  if (!errorInfo.value) return '请稍后重试或刷新页面';
...
const copyErrorInfo = async () => {
  ...
  上下文: ${JSON.stringify(errorInfo.value.context, null, 2)}
    `.trim();
