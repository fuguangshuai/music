    ...
    <div class="music-page">
      <div class="music-header h-12 flex items-center justify-between">
        ...
        <div class="music-close flex-shrink-0 ml-3">
          <i
            class="icon iconfont ri-close-line"
            @click="close"
          ></i>
        </div>
      </div>
    ...
    try {
      ...
      if (data?.songs) {
        ...
        if (!updateComplete) {
          ...
          newSongs = songs.filter((song: unknown) => !loadedIds.value.has(song.id));
