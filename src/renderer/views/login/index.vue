<script lang="ts" setup>
  ...
  const loginPhone = async () => {
    const { data } = await loginByCellphone(phone.value, _password.value);
    if (data.code === 200) {
    ...
    <div class="phone-login">
        ...
        >
          ...
          <div class="phone-page">
            ...
            <input
              ...
              class="phone-input"
              type="password"
              :placeholder="t('login.placeholder._password')"
            />
          </div>
          ...
          <n-button
            class="btn-login"
            @click="loginPhone()"
            >{{ t('login.button.login') }}</n-button
          >
        </div>

        <!-- Cookie登录组件 -->
        <div
          v-else-if="isTokenLogin"
          class="phone"
        >
          <cookie-login />
        </div>
      ...
      <div class="bottom">
        <div class="login-switch">
          <div
            class="title"
            @click="isTokenLogin = !isTokenLogin"
          >
