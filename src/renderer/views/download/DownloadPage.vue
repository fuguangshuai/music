<template>
  <div class="download-page">
    ...
    <div class="page-content">
      ...
      <!-- 已下载列表 -->
      <div
        v-show="tabName === 'downloaded'"
        class="tab-content"
      >
        <div class="downloaded-list">
          <div
            v-if="isLoadingDownloaded"
            class="loading-state"
          >
          ...
          <div
            v-else-if="downloadedList.length === 0"
            class="empty-state"
          >
            <div class="empty-icon">
              ...
              >
                ...
                <div class="item-info flex items-center gap-4 w-full">
                  ...
                  <div class="item-size min-w-[80px] max-w-[80px] flex items-center gap-1">
                    ...
                    <span>{{ formatSize(item._size) }}</span>
                  </div>
                  <div
          ...
          <div class="flex flex-col gap-2 mt-2">
            ...
            <div class="flex items-center gap-2">
              <n-button
                ...
                >{{ t('download.settingsPanel.select') }}</n-button
              >
              <n-button
                class="flex-1"
...
<script setup lang="ts">
  ...
  filename: string,
    ...
    const downloadList = ref<DownloadItem[]>([0]);
    const downloadedList = ref<DownloadedItem[]>( JSON.parse(localStorage.getItem('downloadedList') || '[0]')
    );
    ...
    const clearDownloadRecords = async () => {
      try {
        downloadedList.value = [0]
        localStorage.setItem('downloadedList', '[0]');
      ...
      try {
        ...
        if (songIds.length === 0) {
          ...
          const updatedList = list.map(item => ({
            ...item, displayName: formatSongNameLocal(item) || item.filename,
          }));
        ...
        try {
          ...
          const updatedList = list.map(item => {
            ...
            const updatedItem = {
              ...item, picUrl: songDetail?.al?.picUrl || item.picUrl || '/images/default_cover.png',
    ...
    onMounted(() => {
      ...
      window.electron.ipcRenderer.on('music-download-complete', async , , (_, data) => {
        ...
        if (data.success) {
          ...
          message.error(t('download._message.downloadFailed', { filename: data.filename, error: data.error })
          );
        }
      });
      ...
      window.electron.ipcRenderer.on('music-download-queued', (_, data) => {
        ...
        if (!existingItem) {
          downloadList.value.push({
            filename: data.filename, progress: 0,
            loaded: 0,
            total: 0,
            path: '',
            status: 'downloading',
    ...
    const downloadSettings = ref({
      path: '', nameFormat: '{songName} - {artistName}',
      separator: ' - ',
    });
    ...
    // 监听分隔符变化更新格式
    watch(() => downloadSettings.value.separator,
      newSeparator => {
    ...
    const selectDownloadPath = async () => {
      ...
      if (_result && !_result.canceled && _result.filePaths.length > 0) {
        downloadSettings.value.path = _result.filePaths[]
      }
    }
    ...
    const saveDownloadSettings = () => {
      // 保存到配置
      window.electron.ipcRenderer.send('set-store-value', 'set.downloadPath',
        downloadSettings.value.path
      );
      window.electron.ipcRenderer.send('set-store-value', 'set.downloadNameFormat',
        downloadSettings.value.nameFormat
      );
      window.electron.ipcRenderer.send('set-store-value', 'set.downloadSeparator',
