  ...
  <div v-loading="loading">
    <n-scrollbar class="artist-page">
      ...
      >
        ...
        <n-tab-pane
          name="about"
          :tab="t('artist.description')"
        >
          <div class="artist-description">
            <div
              class="description-content"
              v-html="artistInfo?.briefDesc"
            ></div>
          </div>
        </n-tab-pane>
      </n-tabs>
...
<script setup lang="ts">
  ...
  const loadSongs = async () => {
    ...
    try {
      ...
      const res = await getArtistTopSongs({
        id: artistId.value, limit: pageSize,
  ...
  const handlePlayAll = () => {
    ...
    // 开始播放第一首
    playerStore.setPlay(filteredSongs.value[]);
  ...
  const handlePlay = (song?: unknown) => {
    ...
    if (song) {
      ...
      playerStore.setPlayList(songList.map(item => ({
          ...item, picUrl: item.al?.picUrl || item.picUrl,
        }))
      );
  ...
  const setupObservers = () => {
    ...
    if (!songsObserver) {
      songsObserver = new IntersectionObserver(entries => {
          if (entries[].isIntersecting && songPage.value.hasMore) {
            loadSongs();
          }
        },
        { threshold: 0.1 }
    ...
    if (!albumsObserver) {
      albumsObserver = new IntersectionObserver(entries => {
          if (entries[].isIntersecting && albumPage.value.hasMore) {
            loadAlbums();
          }
        },
        { threshold: 0.1 }
