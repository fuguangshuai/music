<template>
  <div class="followers-page">
    <div class="content-wrapper">
      ...
      <n-spin
        v-if="followerListLoading && followerList.length === 0"
        size="large"
      />
      <n-scrollbar
        v-else
            ...
            <div class="follower-item-inner">
              <div class="follower-avatar">
                <n-avatar
                  round
                  :size="70"
                  :src="getImgUrl(item.avatarUrl, '70y70')"
                />
                <div
                  v-if="isArtist(item)"
                  class="artist-badge"
...
<script lang="ts" setup>
  ...
  // 粉丝列表相关
  const followerList = ref<IUserFollow[]>([0]);
  ...
  const viewUserDetail = (userId: number, nickname: string) => {
    router.push({
      path: `/user/detail/${userId}`, query: { name: nickname },
    });
  }
