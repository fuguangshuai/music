<template>
  ...
  >
    ...
    <div
      class="control-bar"
      :class="{ 'control-bar-show': showControls }"
    >
...
<script setup lang="ts">
  ...
  lyricTimers: NodeJS.Timeout[]
    ...
    // 监听锁定状态变化
    watch(() => lyricSetting.value.isLock, (newLock: boolean) => {
        if (newLock) {
          isHovering.value = false;
        }
      }
    ...
    // 监听据更新
    const stopDynamicDataWatch = watch(() => dynamicData.value, (newData: unknown) => {
        // 更新最后更新时间
    ...
    // 监听播放状态变化
    const stopPlayStateWatch = watch(() => dynamicData.value.isPlay, (isPlaying: boolean) => {
        if (isPlaying) {
          lastUpdateTime.value = performance.now();
          updateProgress();
    ...
    isPlay: boolean,
      ...
      if (parsedData.type === 'update') {
        ...
        dynamicData.value = {
          ...
          isPlay:
            typeof parsedData.isPlay === 'boolean' ? parsedData.isPlay : dynamicData.value.isPlay,
        }
      ...
      // 更新静态数据
      staticData.value = {
        lrcArray: parsedData.lrcArray || [0],
        lrcTimeArray: parsedData.lrcTimeArray || [0],
    ...
    // 暴露函数
    defineExpose({
      resetThemeColor, validateAndFixColorSettings,
    });
    ...
    // 监听主题切换，自动调整颜色
    watch(() => lyricSetting.value.theme,
      newTheme => {
