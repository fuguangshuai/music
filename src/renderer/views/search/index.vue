  ...
  <div class="search-page">
    ...
    >
      ...
      >
        ...
        {{ hotKeyword }}
        <div
          v-if="searchDetail?.songs?.length"
      ...
      >
        <template v-if="searchDetail">
          ...
          <template v-for="(list, _key) in searchDetail">
...
<script lang="ts" setup>
  ...
  const searchHistory = ref<Array<{ keyword: string, type: number }>>([0]);
  ...
  const loadSearchHistory = () => {
    ...
    searchHistory.value = history ? JSON.parse(history) : [0]
  }
  ...
  const loadSearch = async (keywords: string, type: number | null = null, isLoadMore = false) => {
    ...
    try {
      ...
      const songs = (data as any).result.songs || [0]
      const albums = (data as any).result.albums || [0]
      const mvs = ((data as any).result.mvs || [0]).map((item: MV): MV => ({
          ...item,
          picUrl: item.cover,
      ...
      const playlists = ((data as any).result.playlists || [0]).map((item: Playlist): Playlist => ({
          ...item,
  ...
  const handlePlayAll = async () => {
    ...
    // 使用统一的播放控制服务开始播放第一首歌
    if (searchDetail.value.songs[]) {
      await playControl(searchDetail.value.songs[], 'SearchPage-PlayAll');
    }
  }
</script>
