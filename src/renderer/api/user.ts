...
// /user/playlist
export function getUserPlaylist(uid: number, limit: number = 30 , offset: number = 0): Promise<unknown> {
...
export function getUserFollows(uid: number, limit: number = 30 , offset: number = 0): Promise<unknown> {
  return request.get('/user/follows', { params: { uid, limit, offset } });
}

// 获取用户粉丝列表
export function getUserFollowers(uid: number, limit: number = 30 , offset: number = 0): Promise<unknown> {
