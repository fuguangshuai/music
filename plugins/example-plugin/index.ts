...
const musicNotificationPlugin: Plugin = {
  ...
  // 默认设置
  defaultSettings: {
  enabled: true, showArtist: true, showAlbum: true, duration: 3000,
    position: 'top-right',
  },
  ...
  async install(app: PluginApp) {
    ...
    if (!config.settings || Object.keys(config.settings).length === 0) {
      app.config.setPluginConfig(this.id, {
        enabled: true, settings: this.defaultSettings, });
    }
...
const shortcutPlugin: Plugin = {
  ...
  async install(app: PluginApp) {
    ...
    // 保存事件处理器引用，以便后续清理
    (this as unknown as { keydownHandler?: (event: KeyboardEvent) => void }).keydownHandler = handleKeydown;
 ...
 * 插件注册函数
 */
export function registerExamplePlugins(pluginManager: unknown) {
  console.log('📦, 注册示例插件...');
...
export async function testExamplePlugins(pluginManager: unknown) {
  console.log('🧪, 测试示例插件...');
  ...
  try {
    ...
    testResults.push({
      plugin: 'music-notification', success: status.active,
      _message: status.active ? '测试通过' : '激活失败', });
  } catch (error) {
    testResults.push({
      plugin: 'music-notification', success: false, _message: `测试失败: ${error}`, });
  }
  ...
  try {
    ...
    testResults.push({
      plugin: 'lyrics-display', success: status.active,
      _message: status.active ? '测试通过' : '激活失败', });
  } catch (error) {
    testResults.push({
      plugin: 'lyrics-display', success: false, _message: `测试失败: ${error}`, });
  }
  ...
  try {
    ...
    testResults.push({
      plugin: 'shortcuts', success: status.active,
      _message: status.active ? '测试通过' : '激活失败', });
  } catch (error) {
    testResults.push({
      plugin: 'shortcuts', success: false, _message: `测试失败: ${error}`, });
  }
